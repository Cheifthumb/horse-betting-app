...
# ✅ Rank top 3 horses by predicted win probability
...
fixed_winrate_threshold = 0.03   # Only used if 'fixed'
...
for race_id, race_df in data.groupby('Race_ID', sort=False):
    ...
    if not ((3 <= field_size <= 7) or (field_size >= 41)):
        print(f"⏩ Skipping Race {race_id} (Field size = {field_size})")
        continue

    print(f"✅ Processing Race {race_id} with {field_size} horses")
