...
data = pd.read_csv('Betting_Simulation/predicted_win_probabilities.23-25Current.csv')
...
data['Predicted_Rank'] = data.groupby('Race_ID')['Predicted_Win_Probability'] \
...
allowed_predicted_ranks = [1]  # <-- Adjust this list as needed
...
for race_id, race_df in data.groupby('Race_ID', sort=False):
    ...
    # ✅ Save full field size BEFORE filtering
...
if not data.empty:
    ...
    # ✅ Save results
    data.to_csv('betting_simulation/betting_simulation_kelly_23-25.csv', index=False)
    ...
    data['Field_Size_Bin'] = pd.cut(data['Field_Size'], bins=[0,4,5,6,7,8, 10, 13, 20], labels=['1–4','5','6','7','8', '9–10', '11–13', '14+'])
